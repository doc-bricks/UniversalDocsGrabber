"""
Tests für Windows-Store-Artefakte und -Materialien in UniversalDocsGrabber.
...
class TestStoreMaterials(unittest.TestCase):
    ...
    def test_store_package_json_exists_and_is_valid(self):
        ...
        required = [
            ...
            "category", "age_rating", "privacy_url", "support_url",
            "license", "languages"
        ]
    ...
    def test_store_package_publisher_matches_expected(self):
        ...
        self.assertEqual(
            data["identity_name"], EXPECTED_IDENTITY,
            f"identity_name != {EXPECTED_IDENTITY}",
        )
    ...
    def test_store_package_executable_configured(self):
        ...
        data = json.loads(path.read_text(encoding="utf-8"))
        self.assertEqual(
            data["executable"], "UniversalDocsGrabber.exe",
            "executable in store_package.json muss UniversalDocsGrabber.exe sein",
        )
    ...
    def test_appx_manifest_exists_and_valid(self):
        ...
        self.assertTrue(manifest_file.exists(), "AppxManifest.xml fehlt unter store_package/UniversalDocsGrabber/")
        ...
        self.assertEqual(identity.get("Version"), "1.1.7.0")

    def test_store_tile_assets_dimensions(self):
        expected_assets = {
            "store_package/UniversalDocsGrabber/assets/Square44x44Logo.png": (44, 44),
            "store_package/UniversalDocsGrabber/assets/Square50x50Logo.png": (50, 50),
            "store_package/UniversalDocsGrabber/assets/Square150x150Logo.png": (150, 150),
            "store_package/UniversalDocsGrabber/assets/Wide310x150Logo.png": (310, 150),
            "store_package/UniversalDocsGrabber/assets/Square310x310Logo.png": (310, 310),
    ...
    def test_privacy_and_support_docs_exist(self):
        priv_path = PROJECT_ROOT / "PRIVACY.md"
        supp_path = PROJECT_ROOT / "SUPPORT.md"
        self.assertTrue(priv_path.exists(), "PRIVACY.md fehlt")
        self.assertTrue(supp_path.exists(), "SUPPORT.md fehlt")
        self.assertGreater(len(priv_path.read_text(encoding="utf-8").strip()), 100)
        self.assertGreater(len(supp_path.read_text(encoding="utf-8").strip()), 100)
    ...
    def test_store_listing_exists_and_compliant(self):
        ...
        disallowed = ["gmail", "microsoft", "google", "apple", "adobe", "outlook"]
        in_kw = False
        kw_count = 0
    ...
    def test_release_staging_complete(self):
        ws_files = [
            "releases/windowsstore/BUILD.md",
            "releases/windowsstore/WACK_PROTOCOL.md",
            "releases/windowsstore/store_settings.json",
            "releases/windowsstore/store_listing_de.md",
            "releases/windowsstore/store_listing_en.md",
            "releases/windowsstore/StoreLogo.png",
        ]
        for rel_p in ws_files:
            fpath = PROJECT_ROOT / rel_p
            self.assertTrue(fpath.exists(), f"Release-Staging-Datei fehlt: {rel_p}")
            self.assertGreater(fpath.stat().st_size, 10, f"Datei {rel_p} zu klein")
